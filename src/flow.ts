...
export class FlowController {
	...
	public async handle(ctx: AppContext): Promise<boolean> {
		...
		if (!currentState) {
			console.error(
				`State '${currentStateName}' not found in flow '${this.name}'`,
			);
			this.exitFlow(ctx);
			return false;
		}
				...
				if (match) {
					...
					if (match.groups) {
						...
						if (paramNames && match.length > 1) {
							for (
								let i = 0;
								i < paramNames.length && i + 1 < match.length;
								i++
							) {
		...
		if (callbackData && currentState.onAction) {
			if (await processPatterns(callbackData, currentState.onAction))
				return true;
		}
	...
	private async renderState(stateName: string, ctx: AppContext) {
		...
		if (!state) {
			console.error(
				`Cannot render non-existent state '${stateName}' in flow '${this.name}'`,
			);
			this.exitFlow(ctx);
			return;
		}
		...
		try {
			...
			if (isUpdateFromCallback) {
				...
				await ctx.editMessageText(messagePayload.text, {
					reply_markup: messagePayload.reply_markup,
				});
			} else {
				await ctx.reply(messagePayload.text, {
					reply_markup: messagePayload.reply_markup,
				});
			}
		} catch (error) {
			console.error(
				"Zod validation or query execution error in renderState:",
				error,
			);
...
export function createFlow<TConfig extends FlowConfig>(
	...
	const states = Object.keys(config).reduce(
		(acc, key) => {
